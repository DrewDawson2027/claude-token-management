...
[ -f "$TERMINALS_DIR/session-${CLAUDE_SESSION_ID:0:8}.json" ] && exit 0
...
DEBUG_LOG="$TERMINALS_DIR/debug-session-register.log"
...
LINES=$(wc -l < "$TERMINALS_DIR/sessions.jsonl" 2>/dev/null || echo 0)
if [ "$LINES" -gt 200 ]; then
  tail -150 "$TERMINALS_DIR/sessions.jsonl" > "$TERMINALS_DIR/sessions.tmp"
  mv "$TERMINALS_DIR/sessions.tmp" "$TERMINALS_DIR/sessions.jsonl"
fi
